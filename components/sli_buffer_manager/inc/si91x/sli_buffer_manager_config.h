 ...
 * @brief
 * This file contains the configuration for the sli_buffer_manager. for the Si91x family of devices.
 *******************************************************************************
 * # License
...
/**
 * @brief Dedicated memory pool identifiers for the Si91x buffer manager.
 *
 ...
 */
typedef enum {
  SLI_BUFFER_MANAGER_CE_CMD_TX_POOL =
    0,                             ///< Command Engine TX command buffers for host-to-TA frames (Wi-Fi/BLE services).
  SLI_BUFFER_MANAGER_DATA_TX_POOL, ///< Host TX data payload buffers (Wi-Fi/BLE/socket) passed to HAL.
  SLI_BUFFER_MANAGER_CE_METADATA_POOL, ///< Command Engine per-packet metadata (sli_command_engine_metadata_t).
  SLI_BUFFER_MANAGER_HAL_TX_METADATA_POOL, ///< HAL TX queue descriptors that wrap payloads for bus transmission (sli_si91x_hal_packet_t).
  SLI_BUFFER_MANAGER_HAL_CMD_DATA_RX_POOL, ///< HAL bus-interface RX buffers for incoming command and data frames (SPI/UART/AHB).
