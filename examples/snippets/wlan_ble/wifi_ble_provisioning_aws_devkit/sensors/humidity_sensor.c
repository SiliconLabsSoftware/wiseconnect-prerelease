...
void humidity_sensor_init(void)
{
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor reset un-successful, Error Code: 0x%ld ", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor firmware version un-successful, Error Code: 0x%ld ", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor user register 1 write data failed, Error Code: 0x%ld ", status);
    return;
  }
  // Reads register data from sensor
  status = sl_si91x_si70xx_read_control_register(I2C, SI70XX_SLAVE_ADDR, SL_RH_T_USER_REG, &value);
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor user register 1 read failed, Error Code: 0x%ld ", status);
    return;
  }
  osDelay(10);
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor temperature read failed, Error Code: 0x%ld ", status);
    return;
  }
  // measure humidity data from sensor
  status = sl_si91x_si70xx_measure_humidity(I2C, SI70XX_SLAVE_ADDR, &humidity);
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Sensor humidity read failed, Error Code: 0x%ld ", status);
    return;
  }
  osDelay(10);
...
sl_status_t temperature_read(int32_t *temperature)
{
  ...
  if (status != SL_STATUS_OK) {
    SL_DEBUG_LOG_V2(ERROR, "Temperature read failed, Error Code: 0x%ld ", status);
  }
  return status;
}