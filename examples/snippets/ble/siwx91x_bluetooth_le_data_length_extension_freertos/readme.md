...
|------|----------------|-------------------------------------|
| SoC | Application runs on SiWx91x. | `siwx91x_bluetooth_le_soc_data_length_extension_freertos.slcp` |
| PSRAM | Application runs on SiWx91x with PSRAM-capable radio board. | `siwx91x_bluetooth_le_soc_data_length_extension_freertos_psram.slcp` |
| NCP (SPI) | Application runs on **EFR32** host; SiWx917 is the network co-processor over **SPI**. | `siwx91x_bluetooth_le_host_data_length_extension_freertos_spi.slcp` |

Open the `.slcp` for your kit from **`examples/snippets/ble/siwx91x_bluetooth_le_data_length_extension_freertos/`** in Simplicity Studio. For NCP, follow [Getting started with NCP mode](https://docs.silabs.com/wiseconnect/latest/wiseconnect-getting-started/getting-started-with-ncp-mode).
