...
|------------|-----------------------------------------------------------------------------------------|--------------------------------------|
| SoC        | Application runs on SiWx91x.                                                            | `siwx91x_bluetooth_le_soc_ae_central_freertos.slcp`            |
| PSRAM      | Application runs on SiWx91x with PSRAM-capable radio board.                             | `siwx91x_bluetooth_le_soc_ae_central_freertos_psram.slcp`          |
| NCP (SPI)  | Application runs on **EFR32** host; SiWx917 is the network co-processor over **SPI**.   | `siwx91x_bluetooth_le_host_ae_central_freertos_spi.slcp`            |
| NCP (UART) | Application runs on **EFR32** host; SiWx917 NCP over **UART**.                          | `siwx91x_bluetooth_le_host_ae_central_freertos_uart.slcp`       |

Open the `.slcp` for your kit from `examples/snippets/ble/siwx91x_bluetooth_le_ae_central_freertos/` in Simplicity Studio. For NCP, follow [Getting started with NCP mode](https://docs.silabs.com/wiseconnect/latest/wiseconnect-getting-started/getting-started-with-ncp-mode).
...
#### Instructions for Keil IDE and STM32F411RE MCU (NCP Mode)
  ...
  - Open the BLE PER µVision project - **siwx91x_bluetooth_le_ae_central_freertos.uvprojx** by navigating to **WiSeConnect SDK → examples → snippets → ble → siwx91x_bluetooth_le_ae_central_freertos → keil_project**.
