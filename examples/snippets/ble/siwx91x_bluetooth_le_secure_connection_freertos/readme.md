...
|------|----------------|-------------------------------------|
| SoC | Application runs on SiWx91x. | `siwx91x_bluetooth_le_soc_secure_connection_freertos.slcp` |
| PSRAM | Application runs on SiWx91x with PSRAM-capable radio board. | `siwx91x_bluetooth_le_soc_secure_connection_freertos_psram.slcp` |
| NCP (SPI) | Application runs on **EFR32** host; SiWx917 is the network co-processor over **SPI**. | `siwx91x_bluetooth_le_host_secure_connection_freertos_spi.slcp` |

Open the `.slcp` for your kit from **`examples/snippets/ble/siwx91x_bluetooth_le_secure_connection_freertos/`** in Simplicity Studio. For NCP, follow [Getting started with NCP mode](https://docs.silabs.com/wiseconnect/latest/wiseconnect-getting-started/getting-started-with-ncp-mode).
