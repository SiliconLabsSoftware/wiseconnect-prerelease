...
>**Note:**
> Multiple concurrent Paho MQTT client instances are supported over TCP when using the LwIP hosted stack (with or without TLS via mbedTLS). Each `Client` uses its own `Network`/socket.
