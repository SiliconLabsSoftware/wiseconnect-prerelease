...
This application illustrates how to configure the SiWx91x device as an MQTT client using the Paho MQTT library, with all TCP/IP networking managed by the LwIP stack running on the host MCU. Operating in Wi-Fi station mode, the SiWx91x connects to a local access point and utilizes LwIP for socket-level communication. The application walks through the essential steps to establish a secure connection with an MQTT broker, subscribe to specific topics, and publish messages over TCP using SSL/TLS encryption, integrated via mbedTLS.
...
- [Install WiSeConnect extension](https://docs.silabs.com/wiseconnect/latest/wiseconnect-developers-guide-developing-for-silabs-hosts/#install-the-wi-se-connect-3-extension)
...
- Now to publish a message using MQTT Explorer, enter the topic name under **Publish** tab, type the data that you wish to send, and then click on **publish**. This message will be received by the SiWx91x.
...
5. If SSL is enabled, enable **Encryption (TLS)** in MQTT Explorer. Then, go to **Advanced** > **Certificates**, click on **Server certificate (CA)**, and add the CA certificate (`ca.crt`) from the `resources\certificates` directory. Refer to the images below for step-by-step guidance.
