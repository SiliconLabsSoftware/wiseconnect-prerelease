...
>**Note:**
> Multiple concurrent Paho MQTT client instances are supported over TCP (with or without SSL). Each `Client` uses its own `Network`/socket. With SSL enabled, ensure `SL_SI91X_EXT_TCP_IP_FEAT_SSL_THREE_SOCKETS` is set (as in this example) to allow up to three concurrent TLS sockets.
