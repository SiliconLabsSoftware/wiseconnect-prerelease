...
- Configure the following parameter in ``app.c`` to test WPS PBC application.
  ...
  - **WPS Auto-Connect Configuration**
    ...
    - `auto_connect` refers to whether the device should automatically connect to the discovered network after WPS PBC completes, or only retrieve the network credentials for manual connection.
      ...
      - Set `auto_connect false` to only retrieve network credentials without automatically connecting. The application will use the credentials to manually establish the connection.
...
- If `auto_connect` is set to `true`, the device automatically connects to the discovered network. As shown in the image

  ![wps_pbc_auto_connect_true](resources/readme/wps_pbc_auto_connect_true.png)
  
- If `auto_connect` is set to `false`, the application uses the retrieved credentials to manually establish the connection and configure the IP address using DHCP. As shown in the figure
