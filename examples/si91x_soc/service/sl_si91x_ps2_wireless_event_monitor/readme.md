...
This application demonstrates wireless event handling during PS2 power state. It shows how the SiWx917 device can be configured to stay in a low-power mode (PS2) while still maintaining wireless connectivity, allowing the system to process packet events when specific wireless events are detected by transitioning to a high power state to process the packets.
...
- The following example uses a WiFi disconnection event to demonstrate wireless event handling during periodic PS2 to PS3 transitions. 
...
- Upon entering PS2 state, the M4 processor clock is reduced to 2 MHz to further conserve power.
- An OS timer is set to generate an interrupt every 400 ms to transition to the PS3 state to service the pending wireless events. During this transition, IRQs are disabled.
- Before transitioning to the PS3 state, the RC clock frequency is first increased from 2 MHz to 20 MHz. After entering PS3, the system switches to the XTAL clock and scales it up to 40 MHz, ensuring sufficient processing performance for handling wireless events.
...
- OS timer triggers PS2→PS3 transitions every 400ms
