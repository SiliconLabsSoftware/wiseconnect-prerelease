...
static sl_status_t secondary_sync_wait(void);
...
void pcm_secondary_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create pcm_secondary thread\r\n");
    return;
  }
}
...
static void pcm_secondary_task(void *argument)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("PCM driver init failed: 0x%lx, exiting task\r\n", status);
    osThreadExit();
  }
  ...
  if ((osEventFlagsWait(pcm_event_flags, PCM_SECONDARY_EVENT_SEND_COMPLETE, osFlagsWaitAny, osWaitForever)
       & osFlagsError)
      != 0U) {
    DEBUGOUT("PCM send event wait failed\r\n");
    osThreadExit();
  }
  ...
  if ((osEventFlagsWait(pcm_event_flags, PCM_SECONDARY_EVENT_RECEIVE_COMPLETE, osFlagsWaitAny, osWaitForever)
       & osFlagsError)
      != 0U) {
    DEBUGOUT("PCM receive event wait failed\r\n");
    osThreadExit();
  }

  compare_loop_back_data();
  DEBUGOUT("PCM secondary single-shot transfer complete.\r\n");
  osThreadExit();
}
