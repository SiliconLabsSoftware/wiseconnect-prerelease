...
void ssi_master_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create SSI master thread\r\n");
    return;
  }
}
/*******************************************************************************
...
static sl_status_t ssi_master_init_function(void)
{
  ...
  if (ssi_transfer_sem == NULL) {
    DEBUGOUT("SSI semaphore create failed\r\n");
...
static void ssi_master_task(void *argument)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("SSI initialization failed, Error Code : %lu, exiting task\r\n", status);
    osThreadExit();
  }
  ...
  if (SSI_MASTER_TRANSFER) {
    ...
    if (ssi_app_compare_data() != SL_STATUS_OK) {
      DEBUGOUT("SSI master data comparison failed\r\n");
    }
  }
