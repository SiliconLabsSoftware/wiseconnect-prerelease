...
void i2c_leader_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create I2C leader task\r\n");
  }
}
...
static sl_status_t i2c_leader_init_function(void)
{
  ...
  DEBUGOUT("Successfully initialized I2C leader\r\n");
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_enable_repeated_start: Error Code : %u\r\n", i2c_status);
    return SL_STATUS_FAIL;
  }
...
static void i2c_leader_task(void *argument)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2C leader init failed\r\n");
    osThreadExit();
  }
  ...
  DEBUGOUT("I2C leader send completed\r\n");
  ...
  DEBUGOUT("I2C leader receive completed\r\n");

  if (i2c_app_compare_data() != SL_STATUS_OK) {
    DEBUGOUT("I2C leader data comparison failed\r\n");
  }
