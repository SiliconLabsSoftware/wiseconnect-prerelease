...
void usart_async_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create USART async task\r\n");
  }
}
...
static sl_status_t usart_async_init_function(void)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("sl_si91x_usart_init: Error Code : %lu\r\n", status);
    return status;
  }
...
static void usart_async_task(void *argument)
{
  ...
  if (usart_event_flags == NULL) {
    DEBUGOUT("Failed to create USART event flags\r\n");
    osThreadExit();
  }

  status = usart_async_init_function();
  if (status != SL_STATUS_OK) {
    DEBUGOUT("USART init failed\r\n");
    osThreadExit();
  }
  ...
  if ((flags & osFlagsError) != 0U) {
    DEBUGOUT("USART send/receive event wait failed\r\n");
    osThreadExit();
  }
  DEBUGOUT("USART async send and receive completed\r\n");

  if (usart_app_compare_data() != SL_STATUS_OK) {
    DEBUGOUT("USART data comparison failed\r\n");
  }
