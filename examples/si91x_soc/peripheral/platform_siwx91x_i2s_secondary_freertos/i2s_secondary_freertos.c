...
void i2s_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create i2s thread\r\n");
    return;
  }
}
/*******************************************************************************
...
static sl_status_t i2s_secondary_init_function(void)
{
  ...
  if (i2s_event_flags == NULL) {
    ...
    if (i2s_event_flags == NULL) {
      DEBUGOUT("I2S event flags create failed\r\n");
...
static void i2s_secondary_task(void *argument)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2S initialization failed: Error Code : %lu, exiting task\r\n", status);
    osThreadExit();
  }
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2S transmit config fail: Error Code : %lu\r\n", status);
    osThreadExit();
  }
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2S transmit start fail: Error Code : %lu\r\n", status);
    osThreadExit();
  }
  ...
  if ((flags & osFlagsError) != 0U) {
    DEBUGOUT("I2S send event wait failed\r\n");
    osThreadExit();
  }
  ...
  status                        = sl_si91x_i2s_config_transmit_receive(i2s_driver_handle, &i2s_xfer_config);
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2S receive config fail: Error Code : %lu\r\n", status);
    osThreadExit();
  }
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2S receive start fail: Error Code : %lu\r\n", status);
    osThreadExit();
  }
  ...
  if ((flags & osFlagsError) != 0U) {
    DEBUGOUT("I2S receive event wait failed\r\n");
    osThreadExit();
  }

  if (i2s_app_compare_data() != SL_STATUS_OK) {
    DEBUGOUT("I2S secondary data comparison failed\r\n");
  }
