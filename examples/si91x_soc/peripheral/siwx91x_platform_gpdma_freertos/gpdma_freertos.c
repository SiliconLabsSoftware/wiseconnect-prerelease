...
static void transfer_complete_callback(void)
{
  DEBUGOUT("Transfer complete callback\r\n");
  if (gpdma_event_flags != NULL) {
...
static void gpdma_controller_error_callback(void)
{
  DEBUGOUT("GPDMA controller error callback\r\n");
  signal_gpdma_error();
}

static void hresp_error_callback(void)
{
  DEBUGOUT("HRESP error callback\r\n");
  signal_gpdma_error();
}

static void fetch_done_callback(void)
{
  DEBUGOUT("Fetch done callback\r\n");
}
...
static bool gpdma_example_init(void)
{
  ...
  if (gpdma_event_flags == NULL) {
    DEBUGOUT("GPDMA event flags create failed\r\n");
    return false;
  }
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("Allocate channel fail %lu \r\n", status);
    return false;
  }
  DEBUGOUT("Allocate channel success\r\n");

  //Register the callback functions for GPDMA transfer
  status = sl_si91x_gpdma_register_callbacks(gpdma_channel, &callback_t);
  if (status != SL_STATUS_OK) {
    DEBUGOUT("Register callback fail %lu \r\n", status);
    return false;
  }
  DEBUGOUT("Register callback success\r\n");
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("Descriptor allocation failed %lu \r\n", status);
    return false;
  }
  DEBUGOUT("Descriptor allocation success\r\n");
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("Build descriptor fail %lu \r\n", status);
    return false;
  }
  DEBUGOUT("Build descriptor success\r\n");
...
static void gpdma_task(void *argument)
{
  ...
  if (!gpdma_example_init()) {
    DEBUGOUT("GPDMA init failed, exiting task\r\n");
    osThreadExit();
  }

  status = sl_si91x_gpdma_transfer(gpdma_channel, src, dst);
  if (status != SL_STATUS_OK) {
    DEBUGOUT("Transfer_fail %lu \r\n", status);
    osThreadExit();
  }
  ...
  if (((wait_result & osFlagsError) != 0U) || ((wait_result & GPDMA_EVENT_ERROR) != 0U)
    ...
    DEBUGOUT("GPDMA wait failed or incomplete (flags 0x%lx)\r\n", wait_result);
    osThreadExit();
  }
  ...
  if (count == GPDMA_TRANSFER_LENGTH) {
    DEBUGOUT("GPDMA Transfer success\r\n");
  } else {
    DEBUGOUT("GPDMA Transfer fail\r\n");
    osThreadExit();
  }
...
void gpdma_freertos_init(void)
{
  ...
  if (osThreadNew((osThreadFunc_t)gpdma_task, NULL, &attr) == NULL) {
    DEBUGOUT("Failed to create gpdma thread\r\n");
  }
}
