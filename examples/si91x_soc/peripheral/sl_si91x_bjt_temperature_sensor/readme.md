...
This application demonstrates how to use the SI91x BJT temperature sensor to measure die temperature. The example shows the complete process of configuring the ADC, OPAMP, and temperature sensor subsystems to convert the BJT voltage output into temperature readings in the range of -40°C to +125°C.
...
**Key Points:**
- The "temperature sensor" is actually a BJT (Bipolar Junction Transistor) circuit that outputs a voltage proportional to temperature
...
The temperature measurement involves these steps:

1. **Configure OPAMP as Unity Gain Buffer**: The OPAMP provides impedance matching and signal conditioning
2. **Measure Band-gap Reference**: Read the stable ~1.2V reference voltage via ADC
3. **Measure BJT Output**: Read the temperature-dependent BJT voltage via ADC  
...
- The band-gap reference provides a stable voltage that doesn't change with temperature
...
The BJT temperature sensor requires specific configuration of the ADC and OPAMP subsystems. Below are the key parameters and their explanations:

**Configure the following parameters in [`sl_si91x_bjt_temperature_sensor.c`](https://github.com/SiliconLabs/wiseconnect/blob/master/components/device/silabs/si91x/mcu/drivers/unified_api/src/sl_si91x_bjt_temperature_sensor.c) file:**
...
| `POS_IP_OPAMP` | 20 | **YES** | **Must be 20** - This selects OPAMP1 output as input to ADC. Value 20 specifically routes the band-gap reference through OPAMP1. Cannot use other values for temperature measurement. |
| `OPAMP_GAIN_OPAMP` | 51 | **YES** | **Unity gain required** - Value 51 configures the OPAMP for unity gain (1x) which is required for accurate voltage buffering. Other gains will affect calibration. |
...
The BJT temperature sensor uses multiple subsystems that may affect other parts of your application:
