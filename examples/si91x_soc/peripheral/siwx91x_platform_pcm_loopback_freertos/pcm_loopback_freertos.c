...
void pcm_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create pcm_loopback thread\r\n");
    return;
  }
}
...
static sl_status_t pcm_loopback_init_function(void)
{
  ...
  if (pcm_event_flags == NULL) {
    DEBUGOUT("PCM event flags create failed\r\n");
  ...
  status                        = sl_si91x_pcm_config_transmit_receive(pcm_handle, &pcm_xfer_config);
  if (status != SL_STATUS_OK) {
    DEBUGOUT("PCM receive invalid config \r\n");
    return status;
  }
...
static void pcm_loopback_task(void *argument)
{
  ...
  if (init_status != SL_STATUS_OK) {
    DEBUGOUT("PCM initialization failed: Error Code : %lu, exiting task\r\n", init_status);
    osThreadExit();
  }
  ...
  if ((flags & osFlagsError) != 0U) {
    DEBUGOUT("PCM event wait failed\r\n");
    osThreadExit();
  }
  ...
  if ((sl_si91x_pcm_get_transmit_data_count(pcm_handle) != (uint32_t)PCM_BUFFER_SIZE)
    ...
    DEBUGOUT("PCM TX/RX data count mismatch\r\n");
    osThreadExit();
  }
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("PCM send abort failed: 0x%lx\r\n", (unsigned long)status);
    osThreadExit();
  }
  status = sl_si91x_pcm_end_transfer(pcm_handle, SL_I2S_RECEIVE_ABORT);
  if (status != SL_STATUS_OK) {
    DEBUGOUT("PCM receive abort failed: 0x%lx\r\n", (unsigned long)status);
    osThreadExit();
  }
  osThreadExit();
}
