...
void sdio_secondary_mode_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create SDIO secondary thread\r\n");
    return;
  }
}
...
static sl_status_t sdio_secondary_init_function(void)
{
  ...
  if (host_intr_sem == NULL || dma_done_sem == NULL) {
    DEBUGOUT("Failed to create SDIO semaphores\r\n");
...
static void sdio_secondary_mode_task(void *argument)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("SDIO Secondary init failed, exiting task\r\n");
    osThreadExit();
  }
    ...
    switch (current_mode) {
      ...
      case SEND_DATA:
        ...
        if (osSemaphoreAcquire(dma_done_sem, osWaitForever) == osOK) {
          DEBUGOUT("Data is transferred from secondary to host successfully \n");
          send_data_flag = true;
          ...
          if ((osKernelGetTickCount() - tt_start) >= 2000) {
            ...
            DEBUGOUT("\r\nPackets sent: %lu\r\n", (unsigned long)packet_count);
            DEBUGOUT("Total bits sent: %lu \r\n", (unsigned long)(packet_count * XFER_BUFFER_SIZE * 8));
            DEBUGOUT("Time diff: %lu ms \r\n", (unsigned long)(tt_end - tt_start));

            throughput = (packet_count * XFER_BUFFER_SIZE * 8) / ((tt_end - tt_start) / 1000);
            DEBUGOUT("Throughput for secondary->host = %lu bps \r\n", (unsigned long)throughput);
      ...
      case TRANSMISSION_COMPLETED:
        DEBUGOUT("SDIO Secondary transmission completed, exiting task\r\n");
        osThreadExit();
        break;
