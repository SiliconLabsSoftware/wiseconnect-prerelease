...
void gspi_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create gspi thread\r\n");
    return;
  }
}
...
static void gspi_task(void *argument)
{
  ...
  if (init_status != SL_STATUS_OK) {
    DEBUGOUT("GSPI initialization failed: Error Code : %lu, exiting task\r\n", init_status);
    osThreadExit();
  }
  ...
  if (gspi_transfer_sem == NULL) {
    DEBUGOUT("Failed to create gspi semaphore\r\n");
    osThreadExit();
  }
    ...
    switch (current_mode) {
      ...
      case SL_GSPI_TRANSFER_DATA:
        ...
        if (gspi_app_compare_data() != SL_STATUS_OK) {
          DEBUGOUT("GSPI data comparison failed\r\n");
        }
