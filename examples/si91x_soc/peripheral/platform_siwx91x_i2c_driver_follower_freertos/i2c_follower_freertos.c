...
void i2c_follower_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create I2C follower task\r\n");
  }
}
...
static sl_status_t i2c_follower_init_function(void)
{
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_init: Error Code : %u\r\n", i2c_status);
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_set_follower_address: Error Code : %u\r\n", i2c_status);
    return SL_STATUS_FAIL;
  }
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_configure_fifo_threshold: Error Code : %u\r\n", i2c_status);
    return SL_STATUS_FAIL;
  }
...
static void i2c_follower_task(void *argument)
{
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("I2C follower init failed\r\n");
    osThreadExit();
  }
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_receive_data_blocking: Error Code : %u\r\n", i2c_status);
    osThreadExit();
  }
  DEBUGOUT("I2C follower receive completed\r\n");
  ...
  if (i2c_status != SL_I2C_SUCCESS) {
    DEBUGOUT("sl_i2c_driver_send_data_blocking: Error Code : %u\r\n", i2c_status);
    osThreadExit();
  }
  DEBUGOUT("I2C follower send completed\r\n");

  if (i2c_app_compare_data() != SL_STATUS_OK) {
    DEBUGOUT("I2C follower data comparison failed\r\n");
  }
...
static sl_status_t i2c_app_compare_data(void)
{
  for (uint32_t data_index = 0; data_index < I2C_BUFFER_SIZE; data_index++) {
    if (i2c_write_buffer[data_index] != i2c_read_buffer[data_index]) {
      DEBUGOUT("Leader-Follower read-write data comparison failed, Test Case Failed\r\n");
      return SL_STATUS_FAIL;
    }
  }
  DEBUGOUT("Leader-Follower read-write data comparison successful, Test Case Passed\r\n");
  return SL_STATUS_OK;
}
