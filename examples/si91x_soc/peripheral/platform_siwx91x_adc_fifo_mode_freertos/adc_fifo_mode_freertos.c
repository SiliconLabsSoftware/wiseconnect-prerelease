...
void adc_fifo_mode_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create ADC FIFO thread\r\n");
    return;
  }
}
...
static void read_adc_value(uint8_t channel_num)
{
  ...
  if (adc_event_flags != NULL) {
    ...
    if ((int32_t)wait_result < 0) {
      DEBUGOUT("osEventFlagsWait error: 0x%lx\r\n", (unsigned long)wait_result);
  ...
  for (sample_length = 0; sample_length < local_adc_channel_config.num_of_samples[0]; sample_length++) {
    ...
    DEBUGOUT("Channel %d ADC Measured input[%lu] :%0.2fV\r\n", channel_num, (unsigned long)sample_length, (double)vout);
  }
#ifdef DAC_FIFO_MODE_EN
  ...
  if (sl_adc_config.num_of_channel_enable > 0) {
    ...
    DEBUGOUT("DAC successfully sampled\r\n");
    osDelay(MEASUREMENT_DELAY_MS);
  }
  ...
  if (status != SL_STATUS_OK) {
    DEBUGOUT("sl_si91x_dac_stop: Error Code : %lu\r\n", status);
  }
  status = sl_si91x_dac_deinit();
  if (status != SL_STATUS_OK) {
    DEBUGOUT("sl_si91x_dac_deinit: Error Code : %lu\r\n", status);
  } else {
    DEBUGOUT("SL_DAC deinitialization is successful\r\n");
  }
...
static void adc_fifo_mode_task(void *argument)
{
  ...
  if (adc_event_flags == NULL) {
    DEBUGOUT("Failed to create ADC event flags\r\n");
    osThreadExit();
  }
