# Platform SiWx91x USART SYNCHRONOUS SLAVE - FreeRTOS

## Table of Contents

- [Platform SiWx91x USART SYNCHRONOUS SLAVE - FreeRTOS](#platform-siwx91x-usart-synchronous-slave---freertos)
...
- Open **platform_siwx91x_usart_sync_slave_freertos.slcp** project file, select the **Software Component** tab, and search for **USART** in the search bar.
...
1. Open `platform_siwx91x_usart_sync_slave_freertos.slcp` → Software Components → USART.
...
1. Wire clock, TX, and RX between the two boards as listed in the pin tables:
   ...
   - GND ↔ GND
2. Flash `platform_siwx91x_usart_sync_slave_freertos` (this project) on the slave board **first** and reset it.
3. Flash `platform_siwx91x_usart_sync_master_freertos` on the master board and reset it.
