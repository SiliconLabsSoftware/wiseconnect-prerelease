...
void uart_example_init(void)
{
  ...
  if (thread_id == NULL) {
    DEBUGOUT("Failed to create UART task\r\n");
  }
}
...
static void uart_task(void *argument)
{
  ...
  if (uart_event_flags == NULL) {
    DEBUGOUT("Failed to create UART event flags\r\n");
    osThreadExit();
  }

  status = uart_init_function();
  if (status != SL_STATUS_OK) {
    DEBUGOUT("UART init failed\r\n");
    osThreadExit();
  }
  ...
  if ((flags & osFlagsError) != 0U) {
    DEBUGOUT("UART send/receive event wait failed\r\n");
    osThreadExit();
  }
  DEBUGOUT("UART send and receive completed\r\n");

  if (uart_app_compare_data() != SL_STATUS_OK) {
    DEBUGOUT("UART data comparison failed\r\n");
  }
